type DrapOperateOption = {
	container: HTMLElement; // 容器

	rowCopie: number; // 一行占多少份
	columnCopie: number; // 一列占多少份
	margin?: number; // 外边距
	averageWidthOffset?: number; // 行的偏移值
	averageHeightOffset?: number; // 列的偏移值
};
...
type calcItemType = RectInfo & contentType & { offsetX: number;  offsetY: number};
...
export default class DrapOperate {
	...
	public constructor(options: DrapOperateOption) {
	...
	public getAverageWidthOffset() {
		return this.averageWidthOffset || 0;
	}

	public getAverageHeightOffset() {
	...
	// 获取行的偏移值
	getLeftOffset(left: number, n: number) {
		let result = 0;
		if (n !== 0) { // 不是第一个位置
			result = 
				left +  // 本身的 left
	...
	// 获取列的偏移值
	getTopOffset(top: number, n: number) {
		let result = 0;
		if (n !== 0) { // 不是第一行
			result = 
				top +  // 本身的 top
	 ...
	 */
	 public handleMousedown(
			...
			wrapEl: HTMLElement, 
			elList?: HTMLElement[], 
			setPreviewEl?: (el: HTMLElement) => HTMLElement
		) {
		// 容器偏移量
		...
		// 保存原先的事件处理句柄
		const originMousemoveHandle = document.onmousemove || function(e: MouseEvent) {};
		const originMouseupHandle = document.onmouseup || function(e: MouseEvent) {};

		let previewEl: HTMLElement | null = null, // 预览的DOM
		...
		document.onmousemove = (ev: MouseEvent) => {
			...
			if (previewElOldLeft !== left || previewElOldTop !== top) { // 和上一次相同位置则不渲染预览DOM提升性能
				...
				previewEl = wrapEl.cloneNode(true) as HTMLElement;
		...
		document.onmouseup = (e) => {
			...
			// 更新位置
			item.x = parseFloat(left);
			item.y = parseFloat(top);
		};
		// 取消拖拽默认行为
	 ...
	 */
	public handleZoomMouseDown(event: MouseEvent, wrapEl: HTMLElement, elList?: HTMLElement[]) {
		...
		document.onmouseup = (e) => {
			...
			if (elList && elList.length) {
				...
				if (isOverlap) {
					// 位置复原
					wrapEl.style.cssText += `; width: ${oldWidth}px; 
																		height: ${oldHeight}px;`;
					// 更新大小
					item.width = oldWidth;
			...
			// 更新大小
			item.width = upWidth - margin * 2;
			item.height = upHeight - margin * 2;
		};
	}

	// 获取 css 并设置 css 样式
	public getCssStyle(dom: Element, property: string, defaultValue?: any) {
		const cssDescript = window.getComputedStyle(dom);
		return cssDescript.getPropertyValue(property) || defaultValue;
	}

	// 移动的位置是否合法(在父容器之内)
	public validPosttion(wrap: Element, dom: Element) {
	...
	// left 移动位置的边界判断
	public leftBorderJudgment(left: number, wrapEl: HTMLElement): number {
		const margin = this.getMargin();
	...
	// top 移动位置的边界判断
	public topBorderJudgment(top: number, wrapEl: HTMLElement): number {
		const margin = this.getMargin();
		const rect = this.getRectInfo(wrapEl);
	...
	// 缩放宽度边界判断
	public zoomWidthBorderJudgment(width: number, wrapEl: HTMLElement): number {
		...
		let ret = width;
		// 最小占一份
		if (width < averageWidth) ret = averageWidth;
		// 宽度超标
	...
	// 缩放宽度边界判断
	public zoomHeightBorderJudgment(height: number, wrapEl: HTMLElement): number {
		...
		let ret = height;
		// 最小占一份
		if (height < averageHeight) ret = averageHeight;
		// 高度超标
	...
	// 矩形信息
	public getRectInfo(dom: Element): RectInfo {
	...
	// 判断DOM是否位置重叠
	public isDomOverlapList(dom: Element, domList: Element[]) {
	...
	// 判断是否位置重叠
	public isOverlapList(target: calcItemType, otherList: calcItemType[]) {
		const list = otherList.filter((e) => e.id !== target.id);
		const result = list.some((e) => this.isDomOverlap(target,  e));
		return result;
	}

	public isDomOverlap(rect1: RectInfo, rect2: RectInfo) {
	...
	///// 计算大小和位置
	async calculate(list: Array<contentType>, isInsertDOM: boolean = false): Promise<calcResultType> {
		return new Promise(async (resolve, reject) => {

			// 筛选出没有计算位置的数据
			const oldIds = this.cardList.map(e => e.id);
			const newIds = list.map(e => e.id);

			// 获取出最新的数据
			this.cardList = this.cardList.filter(e => newIds.includes(e.id));

			// 需要计算位置的数据
			const calcList = list.filter(e => !oldIds.includes(e.id));


			const calcList: Array<calcItemType> = JSON.parse(JSON.stringify(list));
			...
			for (let i = 0; i < calcList.length; i++) {
				...
				const res = await this.calcAppendPosition(
					item, 
					[...this.cardList, ...result.success],
					prevX, 
					prevY, 
					isInsertDOM
				);
	...
	///// 计算全部的大小和位置
	async calculateAll(list: Array<contentType>, isInsertDOM: boolean = false): Promise<calcResultType> {
		return new Promise(async (resolve, reject) => {
	 ...
	 * @returns 
	 */
	async calcAppendPosition(
		rect: calcItemType, 
		other: Array<calcItemType>, 
		prevX: number = 0, 
		prevY: number = 0, 
		isInsertDOM: boolean = false
	): Promise<calcItemType | void> {
