...
`webpack`: 构建工具[webpack](https://webpack.docschina.org/)
...
`ts-loader`: ts加载器, 用于在 webpack 中编译ts文件

`html-webpack-plugin`: webpack 中html插件, 用来自动创建html文件

`clean-webpack-plugin`: webpack 中的清除插件, 每次构建都会先清除目录

**npm**安装:`npm i -D webpack webpack-cli webpack-dev-server typescript ts-loader clean-webpack-plugin`

根目录下创建 webpack 的配置文件 **webpack.config.js**, 基本配置如下: 
...
根目录下创建 **tsconfig.json**, 配置可以根据自己需要调整

```json
{
    "compilerOptions": {
        "target": "ES2015", // 转换为 ES6
        "module": "ES2015", // 模块规范 ES6
        "strict": true // 开启所有的严格检查
    }
}
```

**package.json** 的 **scripts** 里添加启动命令

```json
// 开启开发服务并打开 chrome 浏览器
...
结合 [babel](https://www.babeljs.cn/) 来对代码进行转换以使其可以兼容到更多的浏览器
...
在 **module.rules** 数组里添加一个对象

```js
{
    // test 指定的是规则生效的文件(正则表达式)
    test: /\.ts$/, // ts 结尾的文件
    // 先用 babel-loader 转换兼容的 js 再用 ts-loader 去转换 ts 代码
    use: [
        {
            // 指定加载器
            loader: "babel-loader",
